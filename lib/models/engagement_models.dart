...
class MembershipPlan {
  ...
  const MembershipPlan({required this.id, required this.name, required this.price, required this.discountPercent, required this.loyaltyMultiplier, required this.prioritySchedulingEnabled, this.status = 'Active', this.billingPeriod = 'monthly'});
  factory MembershipPlan.fromMap(String id, Map<String, dynamic> m) => MembershipPlan(id: id, name: m['name'] ?? id, price: (m['price'] as num?)?.toDouble() ?? 0, discountPercent: (m['discountPercent'] as num?)?.toDouble() ?? 0, loyaltyMultiplier: (m['loyaltyMultiplier'] as num?)?.toDouble() ?? 1, prioritySchedulingEnabled: m['prioritySchedulingEnabled'] ?? false, status: m['status'] ?? 'Inactive', billingPeriod: m['billingPeriod'] ?? 'monthly');
  Map<String, dynamic> toMap() => {'name': name, 'price': price, 'discountPercent': discountPercent, 'loyaltyMultiplier': loyaltyMultiplier, 'prioritySchedulingEnabled': prioritySchedulingEnabled, 'status': status, 'billingPeriod': billingPeriod};
}

class Promotion {
  final String id; final String code; final String type; final double value; final double minimumOrderAmount; final double? maximumDiscount; final bool memberOnly; final int? usageLimit; final int? customerUsageLimit; final DateTime? startDate; final DateTime? endDate; final String status; final String? name;
  const Promotion({required this.id, required this.code, required this.type, required this.value, this.minimumOrderAmount = 0, this.maximumDiscount, this.memberOnly = false, this.usageLimit, this.customerUsageLimit, this.startDate, this.endDate, this.status = 'Active', this.name});
  factory Promotion.fromMap(String id, Map<String, dynamic> m) => Promotion(id: id, code: m['code'] ?? '', type: m['type'] ?? 'percentage', value: (m['value'] as num?)?.toDouble() ?? 0, minimumOrderAmount: (m['minimumOrderAmount'] as num?)?.toDouble() ?? 0, maximumDiscount: (m['maximumDiscount'] as num?)?.toDouble(), memberOnly: m['memberOnly'] ?? false, usageLimit: (m['usageLimit'] as num?)?.toInt(), customerUsageLimit: (m['customerUsageLimit'] as num?)?.toInt(), startDate: _date(m['startDate']), endDate: _date(m['endDate']), status: m['status'] ?? 'Inactive', name: m['name']);
  Map<String, dynamic> toMap() => {'code': code, 'type': type, 'value': value, 'minimumOrderAmount': minimumOrderAmount, 'maximumDiscount': maximumDiscount, 'memberOnly': memberOnly, 'usageLimit': usageLimit, 'customerUsageLimit': customerUsageLimit, 'startDate': startDate, 'endDate': endDate, 'status': status, 'name': name};
